...
<script>
    ...
    async function login() {
      try {
        ...
        } else if (userRoleValue == 1) {
            successmessage = 'Login erfolgreich. Weiterleitung zur Kategorie...';
            console.log("Login erfolgreich");

            setTimeout(() => {
                successmessage = '';
                goto(window.location.pathname); // replace '/kategorie' with the actual path
            }, 3000);
        }
